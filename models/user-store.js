...
const userStore = {
    async addUser(user) {
        const query = 'INSERT INTO station_users (email, first_name, last_name, password) VALUES($1, $2, $3, $4)';
    ...
    async authenticateUser(email, password) {
        const query = 'SELECT * FROM station_users WHERE email=$1 AND password=$2';
    ...
    async getUserById(id) {
        ...
        const query = 'SELECT * FROM station_users WHERE email=$1';
        const values = [id];
        try {
            let dbRes = await dataStoreClient.query(query, values);
